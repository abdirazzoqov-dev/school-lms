...
import { addPartialSalaryPayment } from '@/app/actions/salary'
import { Loader2, DollarSign, Wallet, CreditCard, Banknote } from 'lucide-react'
import { formatNumber } from '@/lib/utils'
import { Progress } from '@/components/ui/progress'

interface AddPartialSalaryModalProps {
  open: boolean
...
export function AddPartialSalaryModal({ open, onOpenChange, payment }: AddPartialSalaryModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await addPartialSalaryPayment(payment.id, amount, paymentMethod, notes)

      if (result.success) {
        toast({
          title: result.isCompleted ? '🎉 Maosh to\'liq to\'landi!' : '✅ To\'lov qo\'shildi',
  ...
  const getPaymentTypeLabel = (type: string) => {
    switch (type) {
      case 'FULL_SALARY': return 'Oylik Maosh'
      case 'ADVANCE': return 'Avans'
      case 'BONUS': return 'Mukofot'
      case 'DEDUCTION': return 'Ushlab qolish'
      default: return type
    }
  }
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-2xl">
            ...
            Bo'lib-bo'lib Maosh To'lash
          </DialogTitle>
          <DialogDescription className="text-base">
            {payment.employeeName} uchun {getPaymentTypeLabel(payment.type).toLowerCase()} to'lash
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="p-4 bg-gradient-to-br from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-xl">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{payment.employeeType === 'teacher' ? 'O\'qituvchi' : 'Xodim'}</p>
              </div>
              <div className="text-right">
                ...
                <p className="text-xs text-muted-foreground">{getPaymentTypeLabel(payment.type)}</p>
              </div>
            </div>
